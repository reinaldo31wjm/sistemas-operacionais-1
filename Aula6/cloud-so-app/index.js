...
app.use(cors(CORS_ORIGIN ? { origin: CORS_ORIGIN } : { origin: '*' }));
...
<head>
  ...
  <meta name="description" content="Dashboard de informações do sistema operacional">
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body {
      ...
      min-height: 100vh;
      color: var(--text);
      background: radial-gradient(circle at top right, #223871 0, transparent 40%), var(--bg);
    }
    ...
    .intro { color: var(--muted); max-width: 690px; line-height: 1.6; margin: 0; }
  ...
  <main>
    <header>
      <div>
        ...
        <p class="intro">Uma visão atualizada do ambiente que está executando este processo Node.js. Em nuvem, os valores representam o host virtualizado ou container disponibilizado pela plataforma.</p>
      </div>
      <div class="status" aria-live="polite">Servidor online</div>
    </header>

    <section class="grid" aria-label="Métricas do sistema">
      <article class="card card-wide">
        <p class="label">Nome do host</p>
        <p class="value" id="hostname">${escapeHtml(snapshot.hostname)}</p>
      </article>
      <article class="card">
        <p class="label">Plataforma</p>
        <p class="value" id="platform">${escapeHtml(snapshot.platform)}</p>
      </article>
      <article class="card">
        <p class="label">Arquitetura</p>
        <p class="value" id="architecture">${escapeHtml(snapshot.architecture)}</p>
      </article>
      <article class="card">
        <p class="label">CPUs lógicas</p>
        <p class="value" id="cpus">${number.format(snapshot.cpus)}</p>
      </article>
      <article class="card">
        <p class="label">Tempo de atividade</p>
        <p class="value" id="uptime">${number.format(snapshot.uptimeMinutes)} <small>min</small></p>
      </article>
      <article class="card card-wide">
        <p class="label">Memória utilizada</p>
        <p class="value"><span id="usedMemory">${number.format(snapshot.memory.usedMB)}</span> <small>MB</small> <span id="usedPercent">(${snapshot.memory.usedPercent}%)</span></p>
        <div class="bar" role="progressbar" aria-label="Memória utilizada" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${snapshot.memory.usedPercent}"><span id="memoryBar" style="width: ${snapshot.memory.usedPercent}%"></span></div>
        <div class="meta"><span>Total: <strong id="totalMemory">${number.format(snapshot.memory.totalMB)} MB</strong></span><span>Livre: <strong id="freeMemory">${number.format(snapshot.memory.freeMB)} MB</strong></span></div>
      </article>
    </section>
  ...
  <script>
    ...
    async function refreshDashboard() {
      try {
        ...
        memoryBar.style.width = data.memory.usedPercent + '%';
        progressbar.setAttribute('aria-valuenow', data.memory.usedPercent);
